# 1. Device diver for VS1053b

This is an audio player capable of handling MP3 files. Development was done
using this [Adafruit breakout](https://www.adafruit.com/product/1381). This
 ...
 * `response` Sets bass boost and/or treble boost/cut. See
 [below](./README.md#531-setting-the-frequency-response).
 ...
 * `SM_LAYER12` Enable MPEG layer 1 and 2 decode. Untested. Probably useless.
 See [section 6](./README.md#6-data-rates).
...
driver, after initialisation, sets the SPI bus baudrate to 8MHz. In practice
the rate may be less than that. Issuing `print(spi_instance)` will show the
actual rate on an individual platform. The Pyboard runs it at 4.5MHz, i.e.
222ns/bit.

For an Nbps data stream the time used by the bus in one second is 222N ns,
which is 28.4ms for a 128Kbps stream. However the stream is handled twice: once
in reading it from the SD card and again in writing it to the VS1053.
Consequently the overhead is 57ms/s or 5.7%. I have successfully tested MP3's
having a 256Kbps rate and VBR files which have a slightly higher rate.

The VS1053 can support lossles FLAC files with a plugin. However the data rate
for FLAC files is about 1Mbps which would give an overhead of 444ms/s or 45%.
This is the irreducibile overhead caused by bus transfers, and takes no account
of the Python code. I think that FLAC playback from the SD card is probably
impossible. The twice as bad WAV format is surely so.
